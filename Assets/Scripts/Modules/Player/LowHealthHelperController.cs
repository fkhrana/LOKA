...
public class LowHealthHelperController : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (playerHealth == null)
            Debug.LogWarning("LowHealthHelperController: Player Health wajib di-assign di Inspector.", this);

        if (helperParent == null)
            Debug.LogWarning("LowHealthHelperController: Helper Parent wajib di-assign di Inspector.", this);
    }
    ...
    private void Start()
    {
        if (playerHealth == null)
        {
            Debug.LogWarning("LowHealthHelperController: PlayerHealth belum diisi.", this);
            return;
        }

        if (helperPrefab == null)
        {
            Debug.LogWarning("LowHealthHelperController: Helper Prefab belum diisi.", this);
            return;
        }

        if (helperParent == null)
        {
            Debug.LogWarning("LowHealthHelperController: Helper Parent belum diisi dan Canvas tidak ditemukan.", this);
            return;
        }
    ...
    public void ConsumeHelper(LowHealthHelper helper)
    {
        ...
        activeHelper = null;
        int healthPerUnit = Mathf.Max(1, Mathf.RoundToInt((float)playerHealth.MaxHealth / Mathf.Max(1, healthUnits)));
